...
public class UnweightedGraph<V> implements Graph<V> {
	...
	public List<Integer> getPath(int u, int v) {
		...
		SearchTree searcher = bfs(v);
		
		int current = u;

		while(searcher.getParent(current) != -1) {
			
			result.add(current);
			
			current = searcher.getParent(current);
		}
	...
	public SearchTree dfsNonRecursive(int v) {
		...
		while(!stack.isEmpty()) {
			...
			if (neighbors.get(current).isEmpty()) {
				...
				for (int n = 0; n < neighbors.get(current).size(); n++) {
					
					int parent = neighbors.get(current).remove(n--).v;
					
					if (isVisited[parent] == false) {
	...
	public List<Integer> getACycle() {
		...
		List<V> vertexes = this.getVertices();
		
		boolean[] visited = new boolean[vertexes.size()];
		
		int[] parents = new int[vertexes.size()];
		...
		while (!vertexes.isEmpty()) {
			...
			while (!vStack.isEmpty()) {
				...
				if (neighbors.isEmpty()) {
					...
					for (int n = 0; n < neighbors.get(x).size(); n++) {
						
					 Edge e = neighbors.get(x).get(n);
